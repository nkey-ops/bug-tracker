...
import static com.bluesky.bugtraker.shared.authorizationenum.Role.ROLE_ADMIN;
import static com.bluesky.bugtraker.shared.authorizationenum.Role.ROLE_SUPER_ADMIN;
import static com.bluesky.bugtraker.shared.authorizationenum.Role.ROLE_USER;
...
public class InitialUserSetup {
  ...
  public void onApplicationEvent(ApplicationReadyEvent readyEvent) {
    ...
    if (!userService.existsUserByEmail(superAdminEmail)) userService.createUserWithRole(userDTO);
  }

  @Transient
  private RoleEntity createRole(Role role) {
    if (roleRepo.findByRole(role).isPresent()) return roleRepo.findByRole(role).get();
    else {
