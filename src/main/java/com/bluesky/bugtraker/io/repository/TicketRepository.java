...
public interface TicketRepository extends DataTablesRepository<TicketEntity, Long> {
  ...
  long countAllByProjectCreatorAndLastUpdateTimeAfter(
      UserEntity projectCreator, Date lastUpdateTime);

  long countAllByProjectCreatorAndSeverity(UserEntity userEntity, Severity severity);

  long countAllByProjectCreatorAndSeverityAndLastUpdateTimeAfter(
      UserEntity projectCreator, Severity severity, Date createdTime);

  long countAllByProjectCreatorAndStatus(UserEntity userEntity, Status status);

  long countAllByProjectCreatorAndStatusAndLastUpdateTime(
      UserEntity projectCreator, Status status, Date lastUpdateTime);
}
