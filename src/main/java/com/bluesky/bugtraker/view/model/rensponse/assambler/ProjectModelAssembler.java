...
public class ProjectModelAssembler
    implements RepresentationModelAssembler<ProjectDTO, ProjectResponseModel> {
  private final Utils utils;
  private final ModelMapper modelMapper;

  public ProjectModelAssembler(Utils utils, ModelMapper modelMapper) {
    this.utils = utils;
  ...
  public ProjectResponseModel toModel(ProjectDTO projectDTO) {
    ProjectResponseModel project = modelMapper.map(projectDTO, ProjectResponseModel.class);

    return project.add(
        linkTo(
                methodOn(ProjectController.class)
                    .getProject(project.getCreator().getPublicId(), project.getPublicId()))
            .withSelfRel(),
        linkTo(
                methodOn(ProjectViewController.class)
                    .getProjectPage(project.getCreator().getPublicId(), project.getPublicId()))
            .withRel("page"),
        linkTo(
                methodOn(ProjectController.class)
                    .getProjects(project.getCreator().getPublicId(), null))
            .withRel("projects"),
        linkTo(methodOn(UserViewController.class).getUserPage(project.getCreator().getPublicId()))
            .withRel("creatorPage"));
  }

  public DataTablesOutput<ProjectResponseModel> toDataTablesOutputModel(
      DataTablesOutput<ProjectDTO> input) {
