...
public class UserServiceUtils {
    ...
    public Map<Severity, Long> getTicketsSeverityOverall(UserEntity userEntity ) {
        ...
        tickets.put(Severity.LOW, ticketRepo.countAllByProjectCreatorAndSeverity(userEntity, Severity.LOW));
        tickets.put(Severity.MINOR, ticketRepo.countAllByProjectCreatorAndSeverity(userEntity, Severity.MINOR));
        tickets.put(Severity.MAJOR, ticketRepo.countAllByProjectCreatorAndSeverity(userEntity, Severity.MAJOR));
        tickets.put(Severity.CRITICAL, ticketRepo.countAllByProjectCreatorAndSeverity(userEntity, Severity.CRITICAL));
        return tickets;
    }
    ...
    public Map<Severity, List<Long>> getTicketsSeverityPerWeek(UserEntity userEntity) {
        ...
        for (int i = 0; i < 5; i++) {
            Specification<TicketEntity> ticketByCreatorAndLastUpdate = ticketByProjectCreator(userEntity).and(lastUpdatedWeeksAgo(i));
            
            long countLow = ticketRepo.count(ticketByCreatorAndLastUpdate.and(severityIs(Severity.LOW)));
            long countMinor = ticketRepo.count(ticketByCreatorAndLastUpdate.and(severityIs(Severity.MINOR)));
            long countMajor = ticketRepo.count(ticketByCreatorAndLastUpdate.and(severityIs(Severity.MAJOR)));
            long countCritical = ticketRepo.count(ticketByCreatorAndLastUpdate.and(severityIs(Severity.CRITICAL)));
    ...
    public Map<Status, List<Long>> getTicketsStatusPerWeek(UserEntity userEntity) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            long countInProgress = ticketRepo.count(ticketByCreatorAndLastUpdate.and(statusIs(Status.IN_PROGRESS)));
            long countCompleted = ticketRepo.count(ticketByCreatorAndLastUpdate.and(statusIs(Status.COMPLETED)));
