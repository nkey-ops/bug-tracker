...
public class ServiceUtilsTest {
  ...
  @InjectMocks private DataExtractionUtils serviceUtils;
  ...
  @Test
  void getUserEntity() {
    when(userRepo.findByPublicId(userEntity.getPublicId())).thenReturn(Optional.of(userEntity));
  ...
  void getUserEntityThrowsWhenUserWasNotFound() {
    when(userRepo.findByPublicId(userEntity.getPublicId())).thenReturn(Optional.empty());

    assertThrows(
        UserServiceException.class, () -> serviceUtils.getUserEntity(userEntity.getPublicId()));
  }
  ...
  void getProjectEntityThrowsWhenProjectWasNotFound() {
    when(projectRepo.findByPublicId(projectEntity.getPublicId())).thenReturn(Optional.empty());
  ...
  void getRoleEntityToBeSet() {
    ...
    RoleEntity actualRoleEntity =
        serviceUtils.getRoleEntityToBeSet(Role.ROLE_ADMIN, Role.ROLE_USER);
  ...
  void getTicketEntityThrowsWhenTicketWasNotFound() {
    when(ticketRepo.findByPublicId(ticketEntity.getPublicId())).thenReturn(Optional.empty());
